...
export function UserTable({ locale, initialPage = 1, initialSearch = "" }: Props) {
  ...
  async function load(p: number, q: string) {
    setLoading(true);
    try {
      const data = await getUsers({ page: p, search: q || undefined });
  ...
  async function handleDeactivate(user: User) {
    ...
    try { await deactivateUser(user.id, reason); await load(page, search); }
  ...
  async function handleReactivate(user: User) {
    ...
    setDeactivating(user.id);
    try { await reactivateUser(user.id); await load(page, search); }
    finally { setDeactivating(null); }
  }

  const totalPages = Math.ceil(total / 20);

  // Show organisation column when data contains it
  const showOrg = users.some((u) => u.organisation);
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-gray-100 bg-gray-50">
                ...
                {showOrg && (
                  <th className="text-start px-4 py-3 font-medium text-gray-600">Organisation</th>
                )}
            ...
            <tbody>
              {loading ? (
                <tr>
                  <td colSpan={showOrg ? 7 : 6} className="px-4 py-8 text-center text-gray-400">
                    Loading...
                  </td>
                </tr>
              ) : users.length === 0 ? (
