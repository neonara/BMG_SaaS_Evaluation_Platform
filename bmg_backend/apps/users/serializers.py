...
class UserCreateSerializer(serializers.ModelSerializer):
    ...
    def create(self, validated_data: dict) -> User:
        ...
        # Priority: explicit tenant_schema (super_admin picks one) →
        #           creator's own tenant_schema (admin_client/hr) →
        #           current connection schema (fallback)
        explicit_schema = validated_data.pop("tenant_schema", None)
        request = self.context.get("request")
        creator_schema = (
            request.user.tenant_schema
            if request and hasattr(request, "user") and request.user.tenant_schema
            else None
        )
        if not explicit_schema:
            from django.db import connection
            explicit_schema = creator_schema or connection.schema_name
        tenant_schema = explicit_schema
